...
// --- 清屏与缓冲区 ---
 ...
 void ClearColor(GLfloat red, GLfloat green, GLfloat blue,
                       GLfloat alpha) {
  glClearColor(red, green, blue, alpha);
}
 void ClearStencil(GLint s) { glClearStencil(s); }
 void ClearDepth(GLdouble depth) { glClearDepth(depth); }
 void DrawBuffer(GLenum buf) { glDrawBuffer(buf); }
 void ReadBuffer(GLenum src) { glReadBuffer(src); }
...
// --- 状态设置 ---
 ...
 void IsEnabled(GLenum cap) { glIsEnabled(cap); }
 ...
 void PointSize(GLfloat size) { glPointSize(size); }
...
// --- 视口与深度范围 ---
 ...
 void DepthRange(GLdouble n, GLdouble f) { glDepthRange(n, f); }
...
// --- 显示列表（Legacy）---
 void NewList(GLuint list, GLenum mode) { glNewList(list, mode); }
 void EndList() { glEndList(); }
 void CallList(GLuint list) { glCallList(list); }
 ...
 GLuint GenLists(GLsizei range) { return glGenLists(range); }
 void ListBase(GLuint base) { glListBase(base); }

// --- 顶点、颜色、法线等（Immediate Mode, Legacy）---
 void Begin(GLenum mode) { glBegin(mode); }
 void End() { glEnd(); }
 void Vertex2d(GLdouble x, GLdouble y) { glVertex2d(x, y); }
 void Vertex2f(GLfloat x, GLfloat y) { glVertex2f(x, y); }
 void Vertex2i(GLint x, GLint y) { glVertex2i(x, y); }
 void Vertex2s(GLshort x, GLshort y) { glVertex2s(x, y); }
 ...
 void TexCoord2f(GLfloat s, GLfloat t) { glTexCoord2f(s, t); }

 void EdgeFlag(GLboolean flag) { glEdgeFlag(flag); }
 void RasterPos2f(GLfloat x, GLfloat y) { glRasterPos2f(x, y); }
